...
global $__settings, $globals, $setupcontinue, $software, $error;
	...
	if(!empty($__settings['admin_pass'])){
		...
		$query = "SELECT `uid` FROM `be_users` WHERE `username` = '".$__settings['admin_username']."';";
		...
		if(empty($userid)){
			...
			//Creating password 
			if(sversion_compare(phpversion(), '7.0', '<')){
				sconfigure('Pbkdf2Salt.php', false, 0, 1);
				...
				$resp = swget($__settings['softurl'].'/Pbkdf2Salt.php');
				...
				sunlink($__settings['softpath'].'/Pbkdf2Salt.php');
			}else{
				$__settings['admin_pass'] = __getHashedPassword($__settings['admin_pass'], PASSWORD_BCRYPT);
			}
			...
			$update_query = "UPDATE `be_users` SET `password` = '".$__settings['admin_pass']."' WHERE `uid` = '".$userid."';";
