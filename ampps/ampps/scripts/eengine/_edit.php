...
global $__settings, $globals, $setupcontinue, $software, $error;
	...
	if(!empty($__settings['admin_pass'])){
		...
		$query = "SELECT `member_id` FROM `".$__settings['dbprefix']."members` WHERE `username` = '".$__settings['admin_username']."';";

		// Does this user exist ?
		$result = sdb_query($query, $__settings['softdbhost'], $__settings['softdbuser'], $__settings['softdbpass'], $__settings['softdb']);
		
		$userid = $result[0]['member_id'];
		...
		if(empty($userid)){
			...
			// Update the password now and also update `salt` in db, as the encryption does not work by fetching old salt and generating new password with older salt
			$update_query = "UPDATE `".$__settings['dbprefix']."members` SET `password` = '".$__settings['admin_pass']."', `salt` = '".$__settings['salt']."' WHERE `member_id` = '".$userid."';";
